...
function StepSection(props){
    ...
    const stepsAmnt = props.stepsAmnt
    const setStepAmnt = props.setStepAmnt

    useEffect(() => {
        let lst=[];
        for(let i = 0; i < stepsAmnt; i++){
            if (steps[i]){
                lst.push(steps[i]);
            } else {
                lst.push(createStep());
    ...
    function handleStepDelete(e, key){
        setSteps(steps.filter(step => {return step.key !== key}));
        setStepAmnt(stepsAmnt - 1);
    }
...
export default function RecipeCreateContainer(prop){
    ...
    const [stepsAmnt, setStepAmnt] = useState(3);
